...
class Services {
  static const String url = 'http://192.168.29.226:8000/news/listscience';
  static Future<List<ScienceClass>> getUsers() async {
    try {
      final response = await http.get(Uri.parse(url));
      if (200 == response.statusCode) {
        final List<ScienceClass> users = scienceClassFromJson(response.body);
        return users;
      } else {
        return [];
      }
